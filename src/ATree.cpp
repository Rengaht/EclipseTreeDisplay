...
ATree::ATree(int index_tree,int index_top,int index_deco1,int index_deco2,int index_deco3,string from_name,string to_name,string word){
	setup(index_tree,index_top,index_deco1,index_deco2,index_deco3,from_name,to_name,word);
					
}
ATree::ATree(string tree_code,string from_name,string to_name,string word){
	
	setup(tree_code[0]-'0',tree_code[1]-'0',tree_code[2]-'0',tree_code[3]-'0',tree_code[4]-'0',from_name,to_name,word);
}




void ATree::setup(int index_tree,int index_top,int index_deco1,int index_deco2,int index_deco3,string from_name,string to_name,string word){
			
	ofLog()<<"New Tree: "<<index_tree<<index_top<<index_deco1<<index_deco2<<index_deco3<<endl
			<<from_name<<" to "<<to_name<<" : "<<word;

	_arr_part_index=new int[5];
	_arr_part_index[0]=index_tree;
	_arr_part_index[1]=index_top;
	_arr_part_index[2]=index_deco1;
	_arr_part_index[3]=index_deco2;
	_arr_part_index[4]=index_deco3;

	_arr_word=new string[3];
	...
	_anim_back_in=new FrameAnimation(stime*1.5);
	_anim_back_video_in=new FrameAnimation(stime*.2,stime*1.2);

	//_anim_back_in->setElastic(true);

	//btime+=200*pace;
	_fly_tree_in=new FlyInAnimation(stime*.5,stime*2,ofVec2f(ofGetWidth()*ofRandom(0,2),-ofGetHeight()*1.4),ofVec2f(0,0));
	...
	_fly_topdeco_in=new FlyInAnimation(stime*.4,stime*3.7,ofVec2f(ofGetWidth()*ofRandom(-.5,.5),-ofGetHeight()*1.2),ofVec2f(0,0));
...
void ATree::update(float dt){
	...
	_display_time-=dt;
	if(!display_finished && _display_time<=0){
		int a=1;
...
void ATree::draw(){
	...
	for(int i=0;i<3;++i){
		...
		else rect=ATreeImage::GetInstance()->_font_LBB_name.getStringBoundingBox(_arr_word[i],0,0);

		float scale_word=.8;
		if(i==1){			
			ATreeImage::GetInstance()->_font_LBB_title.drawString(_arr_word[i],-rect.width/2,rect.height/2);				
		}else if(i==2){
			ATreeImage::GetInstance()->_font_LBB_name.drawString(_arr_word[i],-ATreeImage::GetInstance()->_font_rect.width/2.0,rect.height/2);	
		}else{						
			ATreeImage::GetInstance()->_font_LBB_name.drawString(_arr_word[i],ATreeImage::GetInstance()->_font_rect.width/2.0-rect.width,rect.height/2);	
		}
		ofPopMatrix();
	}
